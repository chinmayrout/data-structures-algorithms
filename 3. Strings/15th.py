...
def findNext(number, n):
    # n = len(number) n
    # Start from the right most digit and find the first digit tha tismaller than the digit next to it
    ...
    # Swapping th above found smallest digit with (i-1)th
...
number = list(map(int, digit))
findNext(number, len(digit))
